...
class Student {
    ...
    getAverageGrade() {
        return `${this.name} (${this.age}) имеет средний балл ${this.grade.reduce((sum, el) => sum += el , 0)/this.grade.length}`;
    }
}
 const res = new Student("Alice", 20, [5, 4, 5, 3, 4]).getAverageGrade();
 console.log(res);
